...
OUTPUT_MODULE = ROOT / "src" / "rbc" / "bids" / "_schema.py"
OUTPUT_TESTS = ROOT / "tests" / "unit" / "bids" / "test_bids.py"
...
def generate_module(  # noqa: C901
    ...
    w()
    w()
    w("def _validate_suffix(suffix: str) -> None:")
    w('    """Validate a BIDS suffix value."""')
    w("    if not _LABEL_RE.fullmatch(suffix):")
    w("        raise ValueError(")
    w('            f"Invalid suffix: {suffix!r}. "')
    w(f'            "Must match: {label_pattern}"')
    w("        )")
    w()
    w()
    w("def _validate_extension(extension: str) -> None:")
    w('    """Validate a BIDS extension value."""')
    ...
    w()
    w()
    w("def _validate_datatype(datatype: str) -> None:")
    w('    """Validate a BIDS datatype value."""')
    w("    if not _LABEL_RE.fullmatch(datatype):")
    w("        raise ValueError(")
    w('            f"Invalid datatype: {datatype!r}. "')
    w(f'            "Must match: {label_pattern}"')
    w("        )")
    w()
    w()
    ...
    # body — validate suffix/extension, then standard entities (except desc)
    w("    _validate_suffix(suffix)")
    w("    _validate_extension(extension)")
...
def generate_tests(
    ...
    # Header
    w(GENERATED_HEADER.rstrip())
    w('"""Unit tests for rbc.bids (auto-generated)."""')
    w()
    w("from pathlib import PurePath")
    w()
    w("import pytest")
    w()
    w("from rbc.bids import (")
    w("    BIDSFile,")
    w("    Datatype,")
    w("    Extension,")
    w("    Modality,")
