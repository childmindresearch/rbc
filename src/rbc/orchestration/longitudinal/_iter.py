"""Shared subject/session iteration primitive for longitudinal orchestration.

Lives in its own module (rather than the package ``__init__``) so per-stage
``run()`` entrypoints can import it without introducing a cycle with the
package root, which re-exports ``process_anat`` / ``process_func``.
"""
...
__all__ = ["iter_sessions_with_template"]
